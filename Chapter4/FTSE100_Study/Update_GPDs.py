...
class Update_GPD:
    ...
    def acceptance_ratio_gpd(self, taus, Z, GPDparams_old, GPDparams_new, xi_sig, scale_sig, pri_nu, pri_kappa, pri_alpha, pri_beta, y):
        new_post = Update_GPD.posterior_gpd(self, taus, Z, GPDparams_new, xi_sig, scale_sig, pri_nu, pri_kappa, pri_alpha, pri_beta, y)
        old_post = Update_GPD.posterior_gpd(self, taus, Z, GPDparams_old, xi_sig, scale_sig, pri_nu, pri_kappa, pri_alpha, pri_beta, y)

        if new_post - old_post > 709:
            return 10000000, new_post, old_post
        else:
            return np.exp(new_post - old_post), new_post, old_post
    ...
    def Update_GPD_Iterator(self, taus, Z, GPDParams, xi_sig, scale_sig, pri_nu, pri_kappa, pri_alpha, pri_beta, DataMat):
        ...
        for j in range(len(taus)-1):
            ...
            r, acc_likval, reg_likval = Update_GPD.acceptance_ratio_gpd(self, [taus[j],taus[j+1]], Z, GPDParams, propedGPD, xi_sig, scale_sig, pri_nu, pri_kappa, pri_alpha, pri_beta, DataMat)
            ...
            if U < rstart:
                ...
                if acc_likval > 0:
                    negloglik.append(acc_likval)
            else:
                if reg_likval > 0:
                    negloglik.append(reg_likval)

        if len(negloglik) != len(taus)-1:
            negloglik = []

        return GPDParams, updated, negloglik
