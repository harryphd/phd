...
class Update_Dim:
    ...
    def accept_split(self, wjstar, xijstar, sigjstar):
        ...
        u2 = expon.rvs(scale=0.1, size=1)[0]
    ...
    def Palloc(self, inds, taus, Data, wj1, wj2, xij1, xij2, sigj1, sigj2):
        ...
        for k in inds:
            ...
            # print(minval, ind_params[0][0], ind_params[1][0], ind_params[0][1], ind_params[1][1])
            ...
            for l in range(len(wjs)):
                denomval = timewindowlikli[l] + const
                if denomval > 709:
                    new_weights.append(np.exp(709) / denom)
                else:
                    new_weights.append(
                        np.exp(denomval) / denom)

            # new_weights = [x.real for x in new_weights]
            # print(new_weights)
    ...
    def Rval(self, Data, taus, inds, xijstar, xi1, xi2, sigjstar, sig1, sig2, wjstar, w1, w2, \
        ...
        if len(Ks1) == 0 or len(Ks2) == 0:
            ...
            if totalis == 0:
                ...
                ck / (sk * Palloc_val) * (
                            beta.pdf(u1, 2, 2) * expon.pdf(u2, scale=0.1) * beta.pdf(u3, 4, 8)) ** (-1) * \
                4 * wjstar * sigjstar
        return R
